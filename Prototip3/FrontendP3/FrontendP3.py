...
class DaoUsuari:
    ...
    def iniciar_sessio(self, correu, contrasenya):
        try:
            ...
            print(f"Respuesta del servidor: {resposta.status_code} - {resposta.text}")

            if resposta.status_code == 200:
                dades = resposta.json()
                if not all(k in dades for k in ['token', 'usuari', 'rol']):
                    return "Error: Respuesta incompleta del servidor"
                ...
                error_msg = resposta.json().get('error', 'Error desconocido')
                return f"Error: {error_msg} (código: {resposta.status_code})"
        ...
        try:
            ...
            if resposta.status_code == 200:
                return [Nen(
                    ...
                    n['data_naixement'],
                    n['informacio_medica']
                ) for n in resposta.json()]
            return f"Error: {resposta.json().get('error', 'Error desconegut')}"
